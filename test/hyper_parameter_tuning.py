...
from models import TAGConvNet,MixConv,HPT_TAGConvNet
...
def my_dataset_loader(task_name,geo_models,train_datasets,test_datasets):
    ...
    test_loader_list = []
    dataset_name_list = []
    if task_name == 'hyper_parameter':
        i_geo = 0
        geo_name = geo_models[i_geo]
        dataset_name = 'hyper_'+geo_name+'_'
    ...
    if task_name == 'valid_wall_with_simu_wall':
        ...
        train_loader = DataLoader(simu_wall_dataset, batch_size=bz, shuffle=True, drop_last=True,
                                num_workers=2)
...
if __name__ == '__main__':
    ...
    for K in [6]:
        for (train_loader,test_loader,dataset_name)  in zip(train_loader_list, test_loader_list,dataset_name_list):
            model = HPT_TAGConvNet(node_features=4, K=K, n_c=n_c,n_G =n_G,n_M=n_M)

            pytorch_total_params = sum(p.numel() for p in model.parameters())
            print("param number: {}".format(pytorch_total_params))
            model_name = 'HPT_TAGConvNet'
            device = torch.device('cuda:1')
            model.to(device)
            optimizer = torch.optim.Adam(model.parameters(), lr=0.001)
            scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=20, gamma=0.5)
            
            exp_name = 'K'+str(K)+'_'+dataset_name+model_name+'with_platform'
            num_epochs = 50
            print(exp_name)
